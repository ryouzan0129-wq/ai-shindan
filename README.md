...
進捗率も残り問題数も表示されません。表示されるのは「回答済み ○○問・プレイ時間・分析中テーマ」だけ。AIは仮説を立て、信頼度を更新し、過去のあなたの回答を引用しながら、次々に新しい分析フェーズを開始します。仮説の信頼度は**絶対に93%を超えません**。つまり、分析は永遠に「もう少しの検証」を必要とします。
...
| `reevalMessages` / `logBank` | 再評価オーバーレイ／画面下AIログ。`{n}` は回答数に置換 |
...
- **ペーシング** … 発火間隔は `script.js` の `Scheduler.init()` にまとまっています（再評価25〜35問、リコール40〜60問、特別演出3〜4テーマ、新仮説4〜5テーマ）

## 設計メモ（この体験の守り方）

- 進捗率・総問題数・残数は**どこにも表示しない**。累積値（回答数・時間）だけを見せる
