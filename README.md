...
「最新AIがあなたを多角的に分析します」——**人生で一番無駄な10〜15分**を過ごすための、友達に送って遊ぶジョークPWAです。
...
静的アセットを変更したら `service-worker.js` 先頭の `CACHE = 'apd-v2.0.0'` のバージョンを上げてください（例: `apd-v2.0.1`）。旧キャッシュは自動削除され、次回アクセスで新版に切り替わります。
