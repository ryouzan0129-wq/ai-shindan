...
- **解析進行度ゲージ**（HUD下）… ゲームの進捗ではなく「AIの解析完了率」。**テーマ終了/分析イベントでのみ**上昇し、回答ごとには動きません。増分は毎回ランダム（例 24→41→56→73→91）で、天井はセッションごとに **94〜95%**。プレイ中に95%超・100%は一瞬も出さず、最後だけ「**解析終了**」の文字に変わります。
...
静的アセットを変更したら `service-worker.js` 先頭の `CACHE` のバージョンを上げてください（現在 `apd-v3.0.0` → 例 `apd-v3.0.1`）。旧キャッシュは自動削除され、次回アクセスで新版に切り替わります。
